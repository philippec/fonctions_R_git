'plot.rdaTest' <- 
	function(x, xax=1, yax=2, scaling=1, graph.type="notchosen", plot.sites=TRUE, 
	plot.spe=TRUE, plot.env=TRUE, binary=NULL, height=6, width=6, xlim=NULL, ylim=NULL, 
	ell=NULL, alpha=0.95, ell.axis=FALSE, mul.spe=0.8, mul.env=0.90, mul.text=0.10, 
...
#
# Version 1.0: SŽbastien Durand and Pierre Legendre, UniversitŽ de MontrŽal, April 2005
...
# Scaling 	
	...
	if(!is.null(binary)) {
		binvar <- matrix(NA,length(binary),ncol(x$Z.mat))
		j=1
		for(i in binary) {
			tmp <- which(x$X.mat[,i]!=0)
			if(length(tmp) == 1) { 
			   binvar[j,] <- x$Z.mat[tmp,] 
			   } else { 
			   binvar[j,] <- apply(x$Z.mat[tmp,]*x$X.mat[tmp,i],2,mean)
			   }
			j=j+1
		}
		colnames(binvar)<-colnames(x$Z.mat)
...
# Draw the triplot
	...
	plot(xinv*x$Z.mat[,xax],yinv*x$Z.mat[,yax],xlim =xlim,ylim=ylim,asp=1,
	    cex = cex, cex.lab = cex.lab, cex.axis = cex.axis,
		xlab=paste("Canonical axis",xax),ylab=paste("Canonical axis",yax),type=type)
	if(label.sites) {
		text(xinv*x$Z.mat[,xax],yinv*x$Z.mat[,yax],rownames(x$Z.mat), 
	...
	mult.spe=multi.factor(x$Z.mat,x$U,xax=xax,yax=yax,xinv=xinv,yinv=yinv,
			percentage=mul.spe)
	mult.env<-multi.factor(x$Z.mat,x$biplotScores1,xax=xax,yax=yax,xinv=xinv,
		yinv=yinv,percentage=mul.env)

	# Draw species (or other response variable) arrows
	if(plot.spe) {
	if(is.null(select.spe)) { vec<-1:nrow(x$U) } else { vec <- select.spe }
	arrows(x0=0, y0=0, x1=xinv*x$U[vec,xax]*mult.spe, y1=yinv*x$U[vec,yax]*mult.spe, 
		col=col.spe, code=2, lty=lty.spe, len=len, lwd=1)
	if(label.spe==TRUE)
		text(x=mul.text*mult.spe*xinv*x$U[vec,xax],y=mul.text*mult.spe*yinv*x$U[vec,yax], 
				rownames(x$U[vec,]), col = col.spe, cex = cex)
	}
	...
	if(!is.null(ell)) {
		...
		for(i in name.ell) {
			numb<-which(ell==i)
			if(label.ell==TRUE) {
				ellipse(XY.mat=cbind(xinv*x$Z.mat[numb,xax],yinv*x$Z.mat[numb,yax]),
