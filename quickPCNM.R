...
cat("\nAIC value for selected model: ",extractAIC(mod)[2],"\n")
...
else {                 # 1.2 op    everything but AIC
  ...
  vars.sign <- sort(fwd.sel[,2])}   # 1.2.1 cl  fwd

    else {                 # 1.2.2 op   all + holm (common part)
...
# At this point, anova.cca for all axes doesn't seem to work: it looses
# the track of one or the other object defined higher (often PCNMred).
# I postpone its use and simply draw the several first axes
...
## Number of axes to draw (arbitrary rule!)
if(mod.test[1,1]<=2){
