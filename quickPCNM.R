...
#   variables (see below).
#
#   This function requires the libraries ape, ade4 and vegan. If regression-based
...
#
#   Y:   an object of class matrix containing response data 
...
#        recommended choice: the ONLY method providing a correct type I error
#        rate for the selection of variables: Blanchet, F.G., P. Legendre and
#        D. Borcard. 2008. Forward selection of explanatory variables. Ecology
#        (accepted 08/01/24).
#     "AIC": Akaike information criterion (very liberal) DON'T USE! Doesn't
#        work properly. Needs reprogramming.
#     "all": test of each PCNM variable in turn, holding all other variables
#        constant (rather conservative)
#     "holm": same as "all" but with Holm correction (conservative). 
...
#        conservative selection method. "fwd" is highly recommended.
...
#          alpha level:
#          name_of_object <- quickPCNM(Y,space,"holm",PCNM,alpha=0.1)
...
#   results.
#   Ignore the warning messages about negative eigenvalues and NaN production:
#   it is the consequence of the PCoA of a non-euclidean distance matrix and
...
#  (largest value of minimum spanning tree)
#  By Stephane Dray

spanning=mst(dist.d1)
dmin <- max(spanning*as.matrix(dist.d1))
                        })
...
if(detrend==TRUE) ({
   temp2 <- rda(Y,space)
   temp2.test <- anova.cca(temp2,alpha=alpha,step=100,perm.max=1000)
...
cat("\nAIC value for selected model: ",extractAIC(mod)[2],"\n")
                               ...
                               # 1.2 op    everything but AIC

  if(method == "fwd") ({   # 1.2.1 op  fwd
  ...
  vars.sign <- sort(fwd.sel[,2])}) else ({   # 1.2.1 cl  fwd
                                             # 1.2.2 op   all + holm (common part)
    ...
    PCNM.pval <- c(rep(0,nb.ev))
      ...
      nb.sig.ev <- length(vars.sign) }) else ({    # 1.3 cl  all
                                                   # 1.4 op  holm
    ...
    cat("\nPCNM variable number ",tab.prob.tri[1,1]," has a p-value of ",tab.prob.tri[1,2])
    cat("\nand will be used alone to build a reduced model.")
    cat("\n------------------------------------------------------------------")
        vars.sign <- tab.prob.tri[1,1]
        nb.sig.ev <- 0
        }) else ({
...
## Axis by axis test to decide how many axes must be drawn
# Works only with model fitting of rda, which doesn't seem to work within this function!
# mod.axes.test <- anova.cca(mod,by="axis",step=100,perm.max=1000)
# 
## Counting of significant axes 
# nb.ax <- 0
# i <- 1
...
## Other possibility: arbitrary rule!
if(mod.test[1,1]<=2) ({
