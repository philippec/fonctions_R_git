"plotKM" <-
function(wrapres,min.g=NULL, max.g=NULL, grpmts.plot=TRUE, sortg=FALSE, gridcol=NA)
{
# DESCRIPTION 

# This function plots the partitions and rearranges the group values so that 
# they form a colour hierarchy

# INPUT
#
# wrapKM<- is the wrap_kmeans output
# The red circle in the plot is the maximum value of the "calinski" criterion
# 
	...
	x<-pregraphKM(x)
	if(sortg==TRUE){
		x<-orderingKM(x) #Reorder the data
#		x<-x[test(x),]
	}else{}
...
#### Partitions graphic window	
	if(grpmts.plot==TRUE){
		def.par <- par(no.readonly = TRUE) 
		nf <- layout(matrix(c(1,2),1,2), widths=c(3,1), TRUE)
		#layout.show(nf)

		par(mar=c(5,5,5,1),bg="white",col="black")
		image(1:nrow(x), 1:nc, x, col=colo, yaxt="n",
			frame.plot=TRUE, main=main, xlab=ylab, ylab=xlab, bg=NA)
	...
	#### Index graphic window
		plot(y=min.g:max.g, x=w[1:nc], type="b", 
			main=paste(criterion,"\ncriterion",sep=""), 
			ylab="",ylim=c(min.g-0.5,max.g+0.5),yaxs="i", yaxt="n",
			xlab="Values")
		grid(nx=NULL,ny=max.g-min.g+1, col=gridcol)
		box()
		axis(2,seq(min.g,max.g,by=1),
			lab=seq(min.g,max.g,by=1), col.axis="black")
		axis(1)
		maxx=which.max(w[])
		minx=which.min(w[])
		...
		if(tolower(criterion)=="calinski"){
			if(!is.na(maxx.o[1]))
				points(y=maxx.o+min.g-1,x=w[maxx.o],col = "orange", pch=19)
			points(y=maxx+min.g-1,x=w[maxx],col = "red", pch=19)
		...
		tops<-which(w[c(2:nc)] - w[c(1:(nc-1))] > 0)+1
		if(length(tops)!=0){
			maxx<-which.max(w[c(2:nc)]-w[c(1:nc-1)])+1
		}else{
			maxx<-which.max(w[])
			tops=1
		}
	}
	return(list(x=x,best.grps=maxx))
}
