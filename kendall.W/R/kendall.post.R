`kendall.post` <-
	function(Y, group, nperm=999, mult="holm", silent=FALSE)
{
...
################################################################################
	...
	a <- system.time({
		...
		for(i in 1:ngr){
			...
			n.per.gr[i] <- length(gr[[i]])  # Vector with the number of variables per group
		}
		...
		for(i in 1:ngr){
			...
			R.gr <- R[,gr[[i]]]# Table with species of group 'i' only as columns
			#CC# Calculate the mean of the Spearman correlations for each species in a group
			spear.mat <- cor(R.gr)
			diag(spear.mat) <- NA
			...
			for(j in 1:p.i){# Test each species in turn
				#CC# Create a new R where the permuted variable has been removed
				R.gr.mod <- R.gr[,-j]
				#CC# Set counter
				counter[[i]][j] <- 1
	...
	a[3] <- sprintf("%2f",a[3])
	if(!silent) cat("Time to compute a posteriori tests (per species) =",a[3]," sec",'\n')
#
	if(ngr == 1) {
